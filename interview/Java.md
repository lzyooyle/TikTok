## 4.23 独占锁，共享锁
+ 独占锁也称为排他锁，意味着在同一时间里，锁只能被一个线程持有，如果线程T加了个排他锁在数据A上，其它线程不可以再再数据A上，添加排他锁了
+ 一个线程获得了排他锁，既可以读也可以写数据，synchronized和JUC的Lock类都是使用的互斥锁
+ 共享锁可以被多个线程所持有，如果线程T申请了共享锁加在数据A上，其它线程只能申请加在数据A上的共享锁，不能申请排他锁，获取共享锁的线程
+ 只能读不能写，排他锁和共享锁也是通过AQS的不同方法实现的
## 4.22 Java多线程实现的方式有四种
+ 1.继承Thread类重写run方法
+ 2.实现Runnable接口，重写run方法，实现Runnable接口的类实例，作为Thread构造器的一个对象
+ 3.使用Callable和FutureTask创建线程
+ 4.通过线程池创建线程
## 4.21 ArrayList 的扩容方式
+ ArrayList底层是一个动态数组，ArrayList首先会评估初始化参数初始化容量，如果参数值是0，会初始化一个空数组，如果不是0，会初始化一个长度为10的数组
+ 初始化容量不能自定义，当增加元素的时候，数组的大小会增加，当数组的长度大于初始化容量的时候，数组会扩容，新的容量是旧的容量的1.5倍
+ 扩容的过程中，旧的数组会被复制到新创建的数组中，旧的数组会被修改指针，指向新的数组，旧的数组会被丢弃和GC回收
...
+ 如果你不想破坏双亲加载机制，只需要重写findClass方法，如果你想要打破双亲委派机制，需要重写整个loadClass方法
...
+ 实现方式：
+ 1.如果知道具体的类，可以使用实例.class的方式
+ 2.通过Object的getClass()方法，实例.getClass()
+ 3.通过class.forName()传入全类路径
+ 4.通过xxxClassLoader.loadClass()
+ 实例化：
+ 1.对于有空构造器的类，可以通过字节码文件直接获得实例，Clazz.newInstance();（如果没有空构造器，这样写会报错）
+ 2.对于没有空构造器的类，需要通过构造器类获得实例前，先获得构造器：
+ Constructor constructor=clazz.getConstructor(String.class,int.class); //得到一个构造器
+ Object obj=constructor.newInstance("jack",18); //通过构造器的实例方法，初始化一个对象

## 自旋锁
自旋锁和互斥锁类似，同一时刻只有一个线程可以获取锁资源，当线程获取锁的时候，如果这把锁已经被另一个线程获取了，这个线程将会循环等待，重复检测锁是否被成功获取
直到获取锁为止，它才会退出循环，在获取锁的时候，线程是活跃的，因此不像互斥锁，自旋锁不会引起线程状态切换（用户态->内核态），它一直是用户态，也就是说
线程一直活跃，它不会让线程进入阻塞状态，减少了不必要的上下文切换，执行速度很快，自旋锁不会释放CPU，如果线程持有自旋锁，不会释放它，线程会等待自旋锁
会持续浪费CPU资源，因此，自旋锁主要适合短时间内持有，线程不会花太多时间重新调度
