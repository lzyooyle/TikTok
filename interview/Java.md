...
+ 2.偏向锁：偏向锁意味着如果一段同步代码一直被一个线程访问，线程自动获得锁，减少了获得锁的开销，大多数情况下
+ 在多数时间里，一把锁总会被一个线程获取，不存在多线程竞争，所以发生偏向锁定时，只有一个线程执行了同步代码块目的是提高性能
+ 当一个线程访问同步代码块获得锁的时候，这把锁偏向的线程id被保存在Mark Word里
+ 没有多线程竞争，因为轻量级锁依靠多个CAS原子指令的获得和释放，然而偏向锁仅依靠一个CAS原子指令用来提交线程id
