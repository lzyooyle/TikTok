...
+ 3.准备
+ 准备阶段是为类文件中个中变量，正式分配内存，设置它们的初始值，方法区的内存空间，被这些分配的变量使用，注意初始值的概念
+ 4.解析
+ 解析阶段涉及，虚拟机将常量池中的符号引用替换成直接引用，符号引用在这些类文件中：
+ 5.初始化
+ 类加载的最后一步初始化阶段，JVM除了加载阶段，可以自定义类加载器，其它都由JVM主导，到初始阶段，定义在这个类中的java程序代码，才会被真正的执行
