...
+ Mark Word：默认保存了对象的HashCode，分代年龄和锁标志信息，这些信息都是对象本身独立定义的，所以Mark Word被设计为非固定的数据结构
+ 用较小的内存空间存储尽可能多的数据，它基于对象的状态复用自己的存储空间，在运行时存储在Mark Word里的数据会随着标志位的改变而改变
+ Class Point：一个指向对象类元数据的指针，虚拟机用来确定这个对象是哪个类的实例
+ Monitor可以理解为一个同步工具或者一个同步机制，通常被描述成一个对象，每个Java对象都有一个不可见的锁，叫做内部锁或者Monitor锁
+ Monitor是线程私有的数据结构，每个线程都有一个可用的monitor记录列表，也称为全局可用记录列表，每个锁对象都有一个monitor
+ monitor有一个Owner字段，保存了拥有该锁线程的唯一标识，表示锁被这个线程占用
+ 目前有四众锁：无锁，偏向锁，轻量级锁，重量级锁，锁状态只能升级不能降级
+ 1.无锁：无锁不用锁资源，所有的线程都可以访问和修改同一个资源，但是只有一个线程能成功修改同一个资源，无锁的特性是循环改变
+ 线程一直尝试修改共享资源，如果没有冲突，修改成功退出，否则循环持续尝试，如果多线程修改相同的值，一个线程成功修改了值
+ 另一个线程会一直尝试，直到修改成功，上面提到的CAS的原则和应用就是无锁实现的，无锁不能完全替代锁，但是无锁在一些场景的性能很高
+ 2.偏向锁：偏向锁意味着一部分同步代码，可以一直被一个线程访问，线程自动获得锁，减少了获得锁的开销，大多数情况下
+ 在多数时间里，一把锁总会被一个线程获取，不存在多线程竞争，所以发生偏向锁定时，只有一个线程执行了同步代码块目的是提高性能
+ 当一个线程访问同步代码块获得锁的时候，这把锁偏向的线程id被保存在Mark Word里，当线程进入或退出同步代码块的时候
+ 不用使用CAS操作区加锁或解锁，Mark Word检测到如果当前的线程的偏向锁被保存了，偏向锁引入了最小非必要轻量级锁执行路径
+ 没有多线程竞争，因为轻量级锁依靠多个CAS原子指令的获得和释放，然而轻量级锁仅依靠一个CAS原子指令用来提交线程id
+ 当其他线程尝试竞争偏向锁的时候，持有偏向锁的线程会释放锁，这个线程将会不再活跃释放偏向锁，撤销偏向锁需要等待一个全局安全指针
+ 没有字节码执行，它会挂起拥有偏向锁的线程，判断这个锁对象是否被锁了，去除锁以后恢复到无锁的状态或者轻量锁状态
+ 偏向锁在jdk6中是默认开启的，偏向锁可以用jvm参数关闭，程序会默认进入轻量级锁状态
+ 3.轻量级锁：当偏向锁被另一个线程访问的时候，这个偏向锁将会被轻量锁更新，其它线程将会通过无锁旋转尝试获取锁，提高性能
+ 当代码进入同步代码块，如果同步对象锁状态是无锁状态，虚拟机将会在当前线程栈中，创建一个名为锁记录的空间
+ 用来保存当前锁对象中Mark Word的拷贝，拷贝这个对象头部锁记录的Mark Word
+ 拷贝成功后，虚拟机将使用CAS操作尝试更新object的Mark Word，并将Lock Record的指针指向对象的Mark Word
+ 如果轻量级锁更新操作失败了，虚拟机首先会检查对象的Mark Word是否指向了当前线程的栈，如果是的，当前线程已经有了锁对象
+ 可以直接进入同步代码块继续操作，否则多线程竞争这把锁，如果当前只有一个等待线程，这个线程会旋转等待
+ 但是轻量级锁会升级为重量级锁，当旋转次数超过一定的次数后，或者一个线程持有了锁，一个在旋转，第三个线程访问
+ 4.重量级锁：当升级为重量级锁时，锁的状态值改为10，重量级锁的指针保存在Mark Word，这时，线程的锁会阻塞
...
+ 2.volatile关键字：valatile关键字确保了变量操作的可见性和有序性
